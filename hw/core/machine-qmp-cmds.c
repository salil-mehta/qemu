...
static CPUArchId *qmp_get_possible_cpu_arch_id(int64_t cpu_index)
{
    MachineState *ms = MACHINE(qdev_get_machine());
...
CPUPowerStateInfoList *qmp_query_cpus_power_state(Error **errp)
{
    ...
    CPU_FOREACH_POSSIBLE(cpu, ms->possible_cpus) {
        CPUArchId *arch_id = cpu ?
            qmp_get_possible_cpu_arch_id(cpu->cpu_index) : NULL;
        ...
        switch (qdev_get_oper_power_state(DEVICE(cpu), errp)) {
