...
void hmp_info_cpus_powerstate(MonitorHMP *mon, const QDict *qdict)
{
    Error *err = NULL;
    ...
    monitor_hmp_printf(mon, "CPUs Power State Info:\n");

    while (entry) {
        CPUPowerStateInfo *cpu = entry->value;

        monitor_hmp_printf(mon, "  CPU ID: %" PRIi64 "\n", cpu->id);

        if (cpu->has_socket_id) {
            monitor_hmp_printf(mon, "    socket-id: %" PRIi64 "\n",
                               cpu->socket_id);
        }
        if (cpu->has_cluster_id) {
            monitor_hmp_printf(mon, "    cluster-id: %" PRIi64 "\n",
                               cpu->cluster_id);
        }
        if (cpu->has_core_id) {
            monitor_hmp_printf(mon, "    core-id: %" PRIi64 "\n", cpu->core_id);
        }
        if (cpu->has_thread_id) {
            monitor_hmp_printf(mon, "    thread-id: %" PRIi64 "\n",
                               cpu->thread_id);
        }
        if (cpu->has_die_id) {
            monitor_hmp_printf(mon, "    die-id: %" PRIi64 "\n", cpu->die_id);
        }
        if (cpu->has_module_id) {
            monitor_hmp_printf(mon, "    module-id: %" PRIi64 "\n",
                               cpu->module_id);
        }
        if (cpu->has_book_id) {
            monitor_hmp_printf(mon, "    book-id: %" PRIi64 "\n", cpu->book_id);
        }
        if (cpu->has_drawer_id) {
            monitor_hmp_printf(mon, "    drawer-id: %" PRIi64 "\n",
                               cpu->drawer_id);
        }
        if (cpu->has_node_id) {
            monitor_hmp_printf(mon, "    node-id: %" PRIi64 "\n", cpu->node_id);
        }
        if (cpu->has_vcpus_count) {
            monitor_hmp_printf(mon, "    vcpus-count: %" PRIi64 "\n",
                               cpu->vcpus_count);
        }
        if (cpu->qom_path) {
            monitor_hmp_printf(mon, "    qom-path: \"%s\"\n", cpu->qom_path);
        }

        monitor_hmp_printf(mon, "    admin-state: \"%s\"\n",
                       CPUAdminPowerState_str(cpu->admin_state));
        monitor_hmp_printf(mon, "    oper-state: \"%s\"\n",
