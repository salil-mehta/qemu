...
static void virt_cpu_set_properties(Object *cpuobj, const CPUArchId *cpu_slot)
{
    ...
    if (vms->secure) {
        object_property_set_link(cpuobj, "secure-memory",
                                 OBJECT(vms->secure_sysmem), &error_abort);
    }
    ...
    if (vms->mte) {
        ...
        if (!vms->tag_sysmem) {
            ...
            vms->tag_sysmem = g_new(MemoryRegion, 1);
            memory_region_init(vms->tag_sysmem, OBJECT(ms),
                               "tag-memory", UINT64_MAX / 32);

            if (vms->secure) {
                vms->secure_tag_sysmem = g_new(MemoryRegion, 1);
                memory_region_init(vms->secure_tag_sysmem, OBJECT(ms),
                                   "secure-tag-memory", UINT64_MAX / 32);

                /* As with ram, secure-tag takes precedence over tag.  */
                memory_region_add_subregion_overlap(vms->secure_tag_sysmem,
                                                    0, vms->tag_sysmem, -1);
            }
        }
    }
...
static void machvirt_init(MachineState *machine)
{
    ...
    if (vms->secure) {
        create_secure_ram(vms, secure_sysmem, vms->secure_tag_sysmem);
        create_uart(vms, VIRT_SECURE_UART, secure_sysmem, serial_hd(1));
    }

    if (vms->tag_sysmem) {
        create_tag_ram(vms->tag_sysmem, vms->memmap[VIRT_MEM].base,
...
static int virt_get_cpu_id_from_cpu_topo(const MachineState *ms, DeviceState *dev)
{
    int cpu_index, sock_vcpu_num, clus_vcpu_num, core_vcpu_num;
    ...
    cpu_index = (sock_vcpu_num + clus_vcpu_num + core_vcpu_num) + cpu->thread_id;
                              ...
                              Error **errp)
{
    ...
    if (qemu_present_cpu(CPU(cpu_slot->cpu))) {
        error_setg(errp, "cpu(id%d=%d:%d:%d:%d) with arch-id %" PRIu64 " exists",
                                    ...
                                    DeviceState *dev, Error **errp)
{
    ...
    {
        error_setg(errp, "hot-unplug of boot cpu(id%d=%d:%d:%d:%d) not supported",
                            ...
                            Error **errp)
{
    ...
    /*
     * RFC: Question:
     * This will not be required if the disabled cpus objects are not released
     * as in current approach.
     */
    if (!dev->always_present)
        cpu_slot->cpu = NULL;
