...
static const MemMapEntry base_memmap[] = {
    ...
    [VIRT_ACPI_CPUHP] =         { 0x090d0000, ACPI_CPU_HOTPLUG_REG_LEN },
...
static inline DeviceState *create_acpi_ged(VirtMachineState *vms)
{
    ...
    if (mc->has_online_capable_cpus) {
        sysbus_mmio_map_name(sbdev, ACPI_CPUHP_REGION_NAME,
                             vms->memmap[VIRT_ACPI_CPUHP].base);
    }
...
virt_cpu_pre_poweron(PowerStateHandler *handler, DeviceState *dev, Error **errp)
{
    ...
    if (*errp) {
        error_prepend(errp, "Could not mark GICC accessible for CPU %d: ",
                      cs->cpu_index);
        return;
    }
    ...
    if (phase_check(PHASE_MACHINE_READY) &&
        ...
        if (*errp) {
            error_prepend(errp, "Failed to notify OSPM about CPU %d power-on: ",
                          cs->cpu_index);
            return;
        }
    }
                          ...
                          Error **errp)
{
    ...
    if (*errp) {
        error_prepend(errp, "Request failed to power off CPU %d: ",
                      cs->cpu_index);
        return;
    }
}
                       ...
                       Error **errp)
{
    ...
    if (*errp) {
        error_prepend(errp, "Failed to complete CPU %d power-off: ",
                      cs->cpu_index);
        return;
    }
    ...
    if (*errp) {
        error_prepend(errp, "Could not mark GICC inaccessible for CPU %d: ",
                      cs->cpu_index);
        return;
    }
