...
static uint32_t virt_acpi_get_gicc_flags(CPUState *cpu)
{
    ...
    const uint32_t GICC_FLAG_ENABLED = 1u; /* bit0 */
    const uint32_t GICC_FLAG_ONLINE_CAPABLE = 1u << 3; /* bit3 */
     ...
     * 'online-capable'. A NULL cpu pointer is treated as non-boot.
     */
    return (cpu->cpu_index == 0) ? GICC_FLAG_ENABLED
