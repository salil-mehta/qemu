...
DEF("smp", HAS_ARG, QEMU_OPTION_smp,
    ...
    "      architectures like ARM that lack architectural CPU hotplug.\n",
    QEMU_ARCH_ALL)
SRST
``-smp [[cpus=]n][,disabledcpus=disabledcpus][,maxcpus=maxcpus][,drawers=drawers][,books=books][,sockets=sockets][,dies=dies][,clusters=clusters][,modules=modules][,cores=cores][,threads=threads]``
    ...
    and '--deviceset' at CLI during boot) are especially useful for platforms
    like ARM that lack native CPU hotplug support. These CPUs will appear to
    the guest as unavailable, and any attempt to bring them online must go
    through QMP/HMP commands like 'device_set'.
