...
struct DeviceListener {
    ...
    /*
     * Resolve user-supplied device options to an existing manageable device.
     *
     * Return the matching manageable DeviceState on success. Return NULL with
     * @errp clear if no existing manageable device matches, so normal qdev
     * creation may continue. If the options identify an existing device that
     * cannot be managed, return NULL with @errp set.
     */
...
/**
 * qdev_find_device() - find an existing manageable device
 *
 * @opts: user-supplied qdev/device_add options
 * @from_json: true if @opts entries are typed, false for keyval strings
 * @errp: pointer to a location where an error can be reported
 *
 * Ask registered device listeners to resolve @opts to an existing manageable
 * device. This is used by the qdev administrative path before falling back to
 * normal qdev object creation.
 *
 * Return: the matching manageable DeviceState on success; NULL with @errp
 * clear if no existing manageable device matches and normal qdev creation
 * should continue; NULL with @errp set if @opts identify an existing device
 * that cannot be managed, or on other lookup errors.
 */
DeviceState *
